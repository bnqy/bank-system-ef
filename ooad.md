...
Here:
- `⌊ x 」` rounding a specified `Decimal` number to the closest integer toward negative infinity (see [Decimal.Floor(Decimal) Method](https://learn.microsoft.com/en-us/dotnet/api/system.decimal.floor));   
...
Here:
- `「 x ⌉` rounding a specified `Decimal` number to the smallest integral value that is greater than or equal to the specified decimal number. (see [Decimal.Ceiling(Decimal) Method](https://learn.microsoft.com/en-us/dotnet/api/system.decimal.ceiling));
- `max(x, y)` gives the maximum of `x` and `y`.
